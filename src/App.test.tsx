...
jest.mock('./store/userActions', () => ({
  fetchUsers: jest.fn().mockImplementation(() => async (dispatch: any) => {
    dispatch({ type: 'FETCH_USERS_SUCCESS', payload: { users: [], currentPage: 1, totalPages: 1 } });
  })
}));
...
describe('App Component', () => {
  ...
  test('renders user grid and pagination when data is loaded', () => {
    mockUseSelector.mockReturnValue({
      users: { 1: [{ id: 1, first_name: 'John', last_name: 'Doe', email: 'john.doe@example.com', avatar: '' }] },
      loading: false,
      error: null,
      currentPage: 1,
      totalPages: 1
